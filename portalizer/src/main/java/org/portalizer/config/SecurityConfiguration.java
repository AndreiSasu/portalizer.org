...
public class SecurityConfiguration extends WebSecurityConfigurerAdapter {
    ...
    public void configure(HttpSecurity http) throws Exception {
        ...
        final HttpSecurity httpSecurity = http
            ...
            if(profiles.containsAny("google-login", "github-login")) {
                httpSecurity.addFilterBefore(ssoFilter(), UsernamePasswordAuthenticationFilter.class);
            }

        ExpressionUrlAuthorizationConfigurer<HttpSecurity>.ExpressionInterceptUrlRegistry urlRegistry = httpSecurity.exceptionHandling()
            .authenticationEntryPoint(problemSupport)
            .accessDeniedHandler(problemSupport)
        .and()
            .headers()
            .contentSecurityPolicy("default-src 'self'; frame-src 'self' data:; script-src 'self' 'unsafe-inline' 'unsafe-eval' https://storage.googleapis.com; style-src 'self' 'unsafe-inline'; img-src 'self' data: *.googleusercontent.com *.githubusercontent.com https://portalizerorg.s3.eu-west-1.amazonaws.com; font-src 'self' data:")
        .and()
    ...
    private Filter ssoFilter() {
        ...
        if(profiles.contains("google-login")) filters.add(googleOAuth2AuthProcessingFilter());
        if(profiles.contains("github-login")) filters.add(githubOAuth2AuthProcessingFilter());
